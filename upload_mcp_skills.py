"""
Upload each Card D skill in skills/ via the Skills API and attach to the right
specialist agent.
...
SKILL_TO_SPECIALIST = {
    "api-analysis-playbook": "api_analyst",
    "mcp-design-patterns": "mcp_architect",
    "typescript-mcp-scaffold": "code_generator",
    "documentation-standards": "documentation_writer",
    "mcp-validation-checklist": "qa_validator",
}
...
def main() -> None:
    ...
    Path(".mcp_skill_ids.json").write_text(json.dumps(uploaded, indent=2))
    print(f"\nUploaded {len(uploaded)} skills and attached them to specialists.")
    print("Next: python run_mcp_builder.py")
